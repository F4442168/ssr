...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		if ! wget -N --no-check-certificate https://github.com/F4442168/master/raw/master/bbr.sh; then
...
Download_SSR(){
	cd "/usr/local/"
	wget -N --no-check-certificate "https://github.com/F4442168/ssr/raw/master/manyuser.zip"
	#git config --global http.sslVerify false
	#env GIT_SSL_NO_VERIFY=true git clone -b manyuser https://github.com/F4442168/shadowsocksr.git
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://github.com/F4442168/ssr/raw/master/ssr_centos -O /etc/init.d/ssr; then
		...
		if ! wget --no-check-certificate https://github.com/F4442168/ssr/raw/master/ssr_debian -O /etc/init.d/ssr; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssr
...
JQ_install(){
	if [[ ! -e ${jq_file} ]]; then
		...
		if [[ ${bit} = "x86_64" ]]; then
			mv "jq-linux64" "jq"
			#wget --no-check-certificate "https://github.com/F4442168/ssr/raw/master/jq-linux64" -O ${jq_file}
		else
			mv "jq-linux32" "jq"
			#wget --no-check-certificate "https://github.com/F4442168/ssr/raw/master/jq-linux32" -O ${jq_file}
		fi
...
Check_Libsodium_ver(){
	...
	Libsodiumr_ver=$(wget -qO- "https://github.com/F4442168/libsodium/tags"|grep "/jedisct1/libsodium/releases/tag/"|head -1|sed -r 's/.*tag\/(.+)\">.*/\1/')
